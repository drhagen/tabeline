...
@pytest.mark.parametrize("expression", ["x * 3", "3 * x"])
def test_multiply_with_positive_literal(dtype, expression):
...
@pytest.mark.parametrize("expression", ["x * -3", "-3 * x"])
def test_multiply_with_negative_literal(original_dtype, expected_dtype, expression):
    df = DataFrame(x=Array[original_dtype](4, None))
    actual = df.transmute(result=expression)
    expected = DataFrame(result=Array[expected_dtype](-12, None))
    assert_data_frames_equal(actual, expected, absolute_tolerance=absolute_tolerance)


@pytest.mark.parametrize(("original_dtype", "expected_dtype"), numeric_to_float)
@pytest.mark.parametrize("expression", ["x * 2.125", "2.125 * x"])
def test_multiply_with_decimal_literal(original_dtype, expected_dtype, expression):
    df = DataFrame(x=Array[original_dtype](4, None))
    actual = df.transmute(result=expression)
    expected = DataFrame(result=Array[expected_dtype](8.5, None))
    assert_data_frames_equal(actual, expected, absolute_tolerance=absolute_tolerance)
