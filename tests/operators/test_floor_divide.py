...
def test_floor_divide_with_constant_integer(dtype, expr, expected):
...
def test_floor_divide_float_with_constant_decimal(dtype, expr, expected):
...
def test_floor_divide_integer_with_constant_decimal(dtype, expr, expected):
    df = DataFrame(x=Array[dtype](2, None))
    actual = df.transmute(result=expr)
